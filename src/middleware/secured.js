...
function handleAuthentication(req, res, next) {
  ...
  if (!req.user && !req.headers.authorization) {
    ...
    if (req.headers.accept.includes('text/html')) {
      console.log(
        `Authentication is required for ${req.originalUrl}. Redirect to login`
      );
    ...
    .then(result => {
      ...
      if (result === null) {
        User.create({ auth0_id: req.user.sub })
          .then(newUser => {
            console.log(`Creating user for Auth0_Id ${req.user.sub}`);
            res.locals.user_id = newUser.id;
            return next();
          })
          .catch(error => {
            console.log(`Error when creating user ${req.user.sub}: ${error}`);
            return next();
          });
      }
      // fetch user
